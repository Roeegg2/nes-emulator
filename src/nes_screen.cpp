...
namespace roee_nes {
NES_Screen::NES_Screen(Mapper *const mapper, Controller *const controller_1,
  ...
  window = SDL_CreateWindow("Roee NES Emulator", SDL_WINDOWPOS_CENTERED,
...
void NES_Screen::draw_pixel_line(const struct Pixel *render_pixel,
                                 const int32_t scanline,
                                 const int32_t x_pos) const {
  SDL_SetRenderDrawColor(renderer, render_pixel->r, render_pixel->g,
                         render_pixel->b, 255);
...
void NES_Screen::update_screen() const {
  SDL_RenderPresent(renderer);
  SDL_Rect viewport = {0, 0, SCREEN_WIDTH * SCALE,
                       SCREEN_HEIGHT *
                           SCALE}; // Assuming screenWidth and screenHeight are
