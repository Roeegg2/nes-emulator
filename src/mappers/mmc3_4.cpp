...
namespace roee_nes {
MMC3_4::MMC3_4(Cartridge *cart) : Mapper(cart) {
  set_irq = false;
  mapper_number = 4;
...
uint8_t MMC3_4::cpu_read(uint16_t addr, uint8_t open_bus_data) {
  ...
  // << "-------------------\n";
  if ((0x6000 <= addr) && (addr <= 0x7fff) &&
      (!(prg_ram_protect & 0b1000'0000)) && (save_ram != nullptr)) {
    ...
    return 0; // should return open bus but fuck it, no MMC3 game uses this
              // anyway
  }
}
