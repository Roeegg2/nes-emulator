...
void CPU::BRK() {
  ...
  PC = convert_to_2byte(bus->cpu_read(0xffff),
                        bus->cpu_read(0xfffe)); // order might be wrong
}
...
/* clear carry bit */
void CPU::CLC() { set_flag(CARRY_BIT, 0); }

/* clear decimal bit - not used in the NES's 6502 implementation, but decided to
 * include this anyway*/
void CPU::CLD() { set_flag(DECIMAL_BIT, 0); }

/* clear interrupt disable bit */
void CPU::CLI() { set_flag(DISINT_BIT, 0); }

/* clear overflow bit */
void CPU::CLV() { set_flag(OVERFLOW_BIT, 0); }

/* compare accumulator with memory */
void CPU::CMP() { reg_CMP_actual(A); }

/* compare X register with memory */
void CPU::CPX() { reg_CMP_actual(X); }

/* compare Y register with memory */
void CPU::CPY() { reg_CMP_actual(Y); }

/* decrement memory */
void CPU::DEC() { mem_INCDEC_actual(-1); }

/* decrement X register */
void CPU::DEX() { reg_INCDEC_actual(X, -1); }

/* decrement Y register */
void CPU::DEY() { reg_INCDEC_actual(Y, -1); }
...
/* increment memory */
void CPU::INC() { mem_INCDEC_actual(1); }

/* increment X register */
void CPU::INX() { reg_INCDEC_actual(X, 1); }

/* increment Y register */
void CPU::INY() { reg_INCDEC_actual(Y, 1); }
...
void CPU::LDA() { reg_LD_actual(A); }

/* load X register with value */
void CPU::LDX() { reg_LD_actual(X); }

/* load Y register with value */
void CPU::LDY() { reg_LD_actual(Y); }
...
/* rotate left */
void CPU::ROL() { // NOTE: I know there is code duplication here, maybe ill fix
                  // later.
  if (inst->mode == ACC)
    ROL_actual(A);
  else {
...
/* set carry bit */
void CPU::SEC() { set_flag(CARRY_BIT, 1); }

/* set decimal bit - not used in the NES's 6502 implementation, but decided to
 * include this anyway*/
void CPU::SED() { set_flag(DECIMAL_BIT, 1); }

/* set interrupt disable bit */
void CPU::SEI() { set_flag(DISINT_BIT, 1); }

/* store accumulator in memory */
void CPU::STA() { bus->cpu_write(bytes, A); }

/* store X register in memory */
void CPU::STX() { bus->cpu_write(bytes, X); }

/* store Y register in memory */
void CPU::STY() { bus->cpu_write(bytes, Y); }

/* transfer accumulator to X register */
void CPU::TAX() { reg_T_actual(X, A); }

/* transfer accumulator to Y register */
void CPU::TAY() { reg_T_actual(Y, A); }

/* transfer stack pointer to X register */
void CPU::TSX() { reg_T_actual(X, S); }

/* transfer X register to accumulator */
void CPU::TXA() { reg_T_actual(A, X); }
