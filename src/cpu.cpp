    ...
    void CPU::fetch_decode_inst() { // this is also ugly, might rewrite it in the future
        ...
        switch (inst->mode) {
            ...
            case IMP:
                ...
                break;
            case ABS:
                log_bytes = bytes = convert_to_2byte(bus->cpu_read(PC + 1), bus->cpu_read(PC + 2));
                PC += 3;
                break;
            case ZP:
            ...
            case ABS_Y:
                ...
                break;
            case IND:
                log_bytes = foo = convert_to_2byte(bus->cpu_read(PC + 1), bus->cpu_read(PC + 2)); // get the 2 next operands (JMP $op1op2)
