...
Mapper *Mapper::create_mapper(const std::string &rom_path) {
  ...
  cart->header.mapper_number =
      (((uint8_t)cart->header.flag_7.parsed.mapper_num_high) << 4) |
      cart->header.flag_6.parsed.mapper_num_low;
  ...
  default:
    std::cerr << "ERROR: Mapper number " << std::dec
              << (int)cart->header.mapper_number << " not implemented yet\n";
    exit(1);
  }
}
...
Cartridge::Cartridge(const std::string &rom_path) {
  ...
  if ((header.flag_9.tv_system == 1) ||
      (header.flag_10.parsed.tv_system ==
       2)) // game is designed for PAL and not NTSC
    std::cerr << "WARNING: This game was designed for the PAL console, this "
                 "emulator is for the NTSC console\n";
  if (header.flag_7.parsed.uu_vsunisystem == 1)
    std::cerr << "WARNING: This game is recommended to play with a different "
                 "palette than the standard one.\n";
  if (header.flag_7.parsed.uu_playchoice == 1)
    std::cerr << "WARNING: This game handles color emphasis differently than "
                 "the standard NES, and this has not been implemented yet\n";

  header.total_size =
      ((header.prg_rom_size * 16) +
       (header.chr_rom_size *
        8)); // the prg and chr banks are in 16kb and 8kb respectively
